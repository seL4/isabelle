...
fun add_attr_text (tagged : Try0.tagged_xref list) (tag, src) s =
  let
    val fs = tagged
      |> map_filter (fn (xref, tags) => if member (op =) tags tag then SOME (Try0_Util.string_of_xref xref) else NONE)
...
fun apply_raw_named_method name (all_goals, attrs) timeout_opt tagged st :
    Try0.result option =
  let
    val st = Proof.map_contexts (Try0_Util.silence_methods false) st
    val unused =
      tagged
      |> filter
