(*  Title:      HOL/ex/Code_Assertion_Ex.thy
    Author:     Florian Haftmann, TU Muenchen
*)

subsection \<open>Lexicographic orders as example for code assertions\<close>

theory Code_Assertion_Ex
  imports "HOL-ex.Code_Assertion"
begin
...
lemma lex_less_simps [simp]:
  \<open>[] [\<^bold><] ys \<longleftrightarrow> ys \<noteq> []\<close>
  ...
  by (auto intro: lex_less.intros elim: lex_less.cases simp add: neq_Nil_conv)

lemma lex_less_eq_simps [simp]:
  \<open>[] [\<^bold>\<le>] ys\<close>
  \<open>xs [\<^bold>\<le>] [] \<longleftrightarrow> xs = []\<close>
...
lemma lex_less_code [code, code_assert in Haskell]: \<comment> \<open>Implicit assertion referring to a code equation\<close>
...
declare [[code_assert: lex_less_eq in Haskell]] \<comment> \<open>Explicit assertion referring to (prospective) constants\<close>
