...
lemma MemReturn_enabled: "\<And>p. basevars (rtrner ch ! p, rs!p) \<Longrightarrow>
  ...
  apply (tactic
    \<open>action_simp_tac (\<^context> |> Simplifier.add_simps @{thms MemReturn_def AReturn_def
      rtrner_def}) [exI] [@{thm base_enabled}, @{thm Pair_inject}] 1\<close>)
  done
...
lemma RNext_enabled: "\<And>p. \<forall>l. basevars (mm!l, rtrner ch!p, rs!p) \<Longrightarrow>
  ...
  apply (case_tac "arg (ch w p)")
   apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm Read_def},
     temp_rewrite \<^context> @{thm enabled_ex}]) [@{thm ReadInner_enabled}, exI] [] 1\<close>)
   apply (force dest: base_pair [temp_use])
  apply (erule contrapos_np)
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm Write_def},
