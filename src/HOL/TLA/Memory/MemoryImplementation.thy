...
lemma HistoryLemma: "\<turnstile> Init(\<forall>p. ImpInit p) \<and> \<box>(\<forall>p. ImpNext p)
  ...
  apply (rule historyI)
      apply assumption+
  apply (rule MI_base)
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simp @{thm HInit_def}) [] [] 1\<close>)
   apply (erule fun_cong)
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simp @{thm HNext_def})
...
lemma S1Hist: "\<turnstile> [HNext rmhist p]_(c p,r p,m p,rmhist!p) \<and> $(S1 rmhist p)
  ...
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps @{thms HNext_def S_def
    S1_def MemReturn_def RPCFail_def MClkReply_def
    AReturn_def}) [] [temp_use \<^context> @{thm squareE}] 1\<close>)
...
lemma S2Forward: "\<turnstile> $(S2 rmhist p) \<and> MClkFwd memCh crCh cst p
  ...
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps @{thms MClkFwd_def
    ACall_def e_def r_def m_def caller_def
    rtrner_def S_def S2_def S3_def Calling_def}) [] [] 1\<close>)
...
lemma S3Forward: "\<turnstile> RPCFwd crCh rmCh rst p \<and> HNext rmhist p \<and> $(S3 rmhist p)
  ...
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps @{thms RPCFwd_def
    HNext_def MemReturn_def RPCFail_def
    MClkReply_def AReturn_def ACall_def e_def
    c_def m_def caller_def rtrner_def S_def
    S3_def S4_def Calling_def}) [] [] 1\<close>)

lemma S3Fail: "\<turnstile> RPCFail crCh rmCh rst p \<and> $(S3 rmhist p) \<and> HNext rmhist p
         \<and> unchanged (e p, c p, m p)
         \<longrightarrow> (S6 rmhist p)$"
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps @{thms HNext_def
    RPCFail_def AReturn_def e_def c_def
    m_def caller_def rtrner_def MVOKBARF_def
    S_def S3_def S6_def Calling_def}) [] [] 1\<close>)
...
lemma S4ReadInner: "\<turnstile> ReadInner rmCh mm ires p l \<and> $(S4 rmhist p) \<and> unchanged (e p, c p, r p)
         ...
         \<longrightarrow> (S4 rmhist p)$ \<and> unchanged (rmhist!p)"
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm ReadInner_def},
...
lemma S4WriteInner: "\<turnstile> WriteInner rmCh mm ires p l v \<and> $(S4 rmhist p) \<and> unchanged (e p, c p, r p) \<and> HNext rmhist p
         \<longrightarrow> (S4 rmhist p)$ \<and> unchanged (rmhist!p)"
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm WriteInner_def},
...
lemma S5Reply: "\<turnstile> RPCReply crCh rmCh rst p \<and> $(S5 rmhist p) \<and> unchanged (e p, c p, m p,rmhist!p)
       \<longrightarrow> (S6 rmhist p)$"
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm RPCReply_def},
...
lemma S5Fail: "\<turnstile> RPCFail crCh rmCh rst p \<and> $(S5 rmhist p) \<and> unchanged (e p, c p, m p,rmhist!p)
         \<longrightarrow> (S6 rmhist p)$"
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm RPCFail_def},
...
lemma S6Retry: "\<turnstile> MClkRetry memCh crCh cst p \<and> HNext rmhist p \<and> $S6 rmhist p
  ...
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm HNext_def},
...
lemma Step1_2_1: "\<turnstile> [HNext rmhist p]_(c p,r p,m p, rmhist!p) \<and> ImpNext p
  ...
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simp @{thm ImpNext_def}) []
...
lemma Step1_2_4: "\<turnstile> [HNext rmhist p]_(c p,r p,m p, rmhist!p) \<and> ImpNext p
  ...
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simp @{thm RNext_def}) []
    (@{thm squareE} ::
...
lemma Step1_3: "\<turnstile> S1 rmhist p \<longrightarrow> PInit (resbar rmhist) p"
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm resbar_def},
...
lemma Step1_4_2: "\<turnstile> MClkFwd memCh crCh cst p \<and> $S2 rmhist p \<and> (S3 rmhist p)$
  ...
  by (tactic \<open>action_simp_tac
    (\<^context> |> Simplifier.add_simps [@{thm MClkFwd_def}, @{thm e_def}, @{thm r_def}, @{thm m_def},
...
lemma Step1_4_3a: "\<turnstile> RPCFwd crCh rmCh rst p \<and> $S3 rmhist p \<and> (S4 rmhist p)$
  ...
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm e_def},
...
lemma Step1_4_4a1: "\<turnstile> $S4 rmhist p \<and> (S4 rmhist p)$ \<and> ReadInner rmCh mm ires p l
  ...
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm ReadInner_def},
    @{thm GoodRead_def}, @{thm BadRead_def}, @{thm e_def}, @{thm c_def}, @{thm m_def}]) [] [] 1\<close>)
     apply (auto simp: resbar_def)
       apply (tactic \<open>ALLGOALS (action_simp_tac
                (\<^context> |> Simplifier.add_simps [@{thm RPCRelayArg_def}, @{thm MClkRelayArg_def},
...
lemma Step1_4_4b1: "\<turnstile> $S4 rmhist p \<and> (S4 rmhist p)$ \<and> WriteInner rmCh mm ires p l v
  ...
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps
    ...
    @{thm c_def}, @{thm m_def}]) [] [] 1\<close>)
     apply (auto simp: resbar_def)
    apply (tactic \<open>ALLGOALS (action_simp_tac (\<^context> |> Simplifier.add_simps
...
lemma Step1_4_6b: "\<turnstile> MClkRetry memCh crCh cst p \<and> $S6 rmhist p \<and> (S3 rmhist p)$
  ...
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm e_def}, @{thm r_def},
...
lemma S1_RNextdisabled: "\<turnstile> S1 rmhist p \<longrightarrow>
  ...
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm angle_def},
...
lemma S1_Returndisabled: "\<turnstile> S1 rmhist p \<longrightarrow>
  ...
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm angle_def}, @{thm MemReturn_def},
...
lemma MClkReplyS6:
  ...
  by (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps [@{thm angle_def},
...
lemma S6MClkReply_enabled: "\<turnstile> S6 rmhist p \<longrightarrow> Enabled (<MClkReply memCh crCh cst p>_(c p))"
  apply (auto simp: c_def intro!: MClkReply_enabled [temp_use])
    ...
    apply (tactic \<open>ALLGOALS (action_simp_tac (\<^context>
      |> Simplifier.add_simps [@{thm S_def}, @{thm S6_def}]) [] [])\<close>)
  done
...
lemma S1Infinite: "\<lbrakk> sigma \<Turnstile> \<not>S1 rmhist p \<leadsto> S6 rmhist p;
  ...
  apply (tactic \<open>asm_lr_simp_tac (\<^context> |> Simplifier.add_simps
