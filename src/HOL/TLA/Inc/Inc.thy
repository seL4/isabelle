...
lemma g1_leadsto_a1:
  ...
  apply (rule SF1)
    apply (tactic
      \<open>action_simp_tac (\<^context> |> Simplifier.add_simps @{thms Psi_defs}) [] [@{thm squareE}] 1\<close>)
   apply (tactic
      \<open>action_simp_tac (\<^context> |> Simplifier.add_simps @{thms angle_def Psi_defs}) [] [] 1\<close>)
...
lemma g2_leadsto_a2:
  ...
  apply (rule SF1)
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps @{thms Psi_defs}) [] [@{thm squareE}] 1\<close>)
  apply (tactic \<open>action_simp_tac (\<^context> |> Simplifier.add_simps @{thms angle_def Psi_defs})
    [] [] 1\<close>)
