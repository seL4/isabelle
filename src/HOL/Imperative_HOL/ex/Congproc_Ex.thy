(*  Title:      HOL/Imperative_HOL/ex/Congproc.thy
...
congproc_setup passive if_cong (\<open>if x then a else b\<close>) = \<open>
  (K (K (K (SOME (Simpdata.mk_meta_cong @{context} @{thm if_cong})))))
\<close>

experiment
begin
text \<open>The congproc takes precedence over the cong rules\<close>
declare [[congproc add: if_cong, simp_trace = false]]
ML_val \<open>
...
congproc_setup passive if_weak_cong_bad (\<open>if x then a else b\<close>) = \<open>
  (K (K (K (SOME (Simpdata.mk_meta_cong @{context} @{thm if_weak_cong})))))
\<close>
...
declare [[congproc add: if_weak_cong_bad, simp_trace]]
...
congproc_setup passive weak if_weak_cong (\<open>if x then a else b\<close>) = \<open>
  (K (K (K (SOME (Simpdata.mk_meta_cong @{context} @{thm if_weak_cong})))))
\<close>
...
begin
declare if_weak_cong [cong del] 
declare [[congproc add: if_weak_cong, simp_trace]]
ML_val \<open>
@{cterm "if True then (1::nat) + 2 else 2 + 3"}
  |> Simplifier.asm_full_rewrite @{context}
  |> assert @{cterm "Suc (Suc (Suc 0))"}
...
simplified.\<close>
congproc_setup passive if_cong_canonical (\<open>if x then a else b\<close>) = \<open>
  let
    val if_True_weak_cong = Simpdata.mk_meta_cong @{context} @{thm if_True_weak_cong}
    val if_False_weak_cong = Simpdata.mk_meta_cong @{context} @{thm if_False_weak_cong}
    val if_cong = Simpdata.mk_meta_cong @{context} @{thm if_cong}
  in
    (K (fn ctxt => fn ct =>
      let
...
begin
declare if_weak_cong [cong del] 
declare [[congproc add: if_cong_canonical, simp_trace]]
ML_val \<open>
@{cterm "if True then (1::nat) + 2 else 2 + 3"}
  |> Simplifier.asm_full_rewrite @{context}
  |> assert @{cterm "Suc (Suc (Suc 0))"}
...
congproc_setup passive if_cong_bad (\<open>if x then a else b\<close>) = \<open>
  let
    val if_True_cong = Simpdata.mk_meta_cong @{context} @{thm if_True_cong}
    val if_False_cong = Simpdata.mk_meta_cong @{context} @{thm if_False_cong}
    val if_cong = Simpdata.mk_meta_cong @{context} @{thm if_cong}
  in
    (K (fn ctxt => fn ct =>
      let
...
begin
declare if_weak_cong [cong del] 
declare [[congproc add: if_cong_bad, simp_trace]]
...
begin
declare if_weak_cong [cong del] 
declare [[congproc add: if_cong, simp_trace]]
...
congproc_setup passive if_cong_diy (\<open>if x then a else b\<close>) = \<open>
  let
    val if_True_diy_cong = Simpdata.mk_meta_cong @{context} @{thm if_True_diy_cong}
    val if_False_diy_cong = Simpdata.mk_meta_cong @{context} @{thm if_False_diy_cong}
    val if_cong = Simpdata.mk_meta_cong @{context} @{thm if_cong}
  in
    (K (fn ctxt => fn ct =>
      let
...
begin
declare if_weak_cong [cong del] 
declare [[congproc add: if_cong_diy, simp_trace]]
...
congproc_setup passive assert_bind_cong (\<open>(assert P x) \<bind> f\<close>) = \<open> 
  K (K (K (SOME (Simpdata.mk_meta_cong @{context} @{thm assert_bind_cong}))))
\<close>

congproc_setup passive assert_bind_cong2 (\<open>(assert P x) \<bind> f\<close>) = \<open> 
  K (K (K (SOME (Simpdata.mk_meta_cong @{context} @{thm assert_bind_cong2}))))
\<close>

experiment
begin
declare [[congproc add: assert_bind_cong]]
...
experiment fixes a::nat
begin
declare [[congproc add: assert_bind_cong2]]
...
function to type @{ML_type "Proof.context -> thm -> (thm list * Proof.context)"} to give the user the
control to do additional modifications to the context: 
@{ML Simplifier.context_mksimps}, @{ML Simplifier.map_context_mksimps}
...
text \<open>We modify @{ML Simplifier.context_mksimps} to derive a theorem about \<^term>\<open>UNPROTECT P\<close> from
 \<^term>\<open>EXTRACT P\<close> and add it to the named theorems @{thm my_theorems}.\<close>

setup \<open>
let
  fun my_mk mk ctxt thm = 
    let
      val (thms, ctxt') = mk ctxt thm
...
in
 Context.theory_map (Simplifier.map_ss (Simplifier.map_context_mksimps my_mk))
end
\<close>
...
ML \<open>
val conproc1 = \<^congproc_setup>\<open>passive weak if_cong1  ("if x then a else b") = 
  \<open>(K (K (K (SOME (Simpdata.mk_meta_cong @{context} @{thm if_cong})))))\<close>\<close>
