...
next
  ...
  have gB: "(\<cdot>) (inverse g) ` B \<in> \<Omega> n"
    using B \<phi>_left_closed assms invertible invertible_inverse_closed by meson
  have "\<phi>_left n g (\<phi>_left n (inverse g) B) = (\<cdot>) g ` ((\<cdot>) (inverse g) ` B)"
    using B gB \<phi>_left_apply by presburger
  also have "\<dots> = B"
    using BG assms invertible invertible_inverse_closed invertible_inverse_inverse
      left_mult_image_inverse by metis
...
next
  ...
  show "\<phi>_left n (inverse g) ` \<Omega> n \<subseteq> \<Omega> n"
    by (simp add: \<phi>_left_closed assms image_iff image_subset_iff)
qed
...
proof (rule ext)
  ...
  proof (cases "B \<in> \<Omega> n")
    ...
    proof -
      ...
      then show ?thesis
        using associative [OF assms]
        by (smt (cvc5, best) compose_eq image_cong in_mono surj_compose)
    qed
...
proof (induction "card G" arbitrary: G "(\<cdot>)" "\<one>" rule: less_induct)
  case less
  then interpret G: Abelian_Group G composition unit by simp
  have "card G \<noteq> 1"
    using less.prems prime by force
  with less obtain a where a: "a\<in>G" "a \<noteq> unit"
    using G.unit_closed by (metis empty_iff is_singletonI' is_singleton_altdef)
  let ?r = "G.element_order a"
  show ?case
  proof (cases "p dvd ?r")
    case True
    then obtain r' where r': "?r = p * r'" and pow_eq: "G.power (G.power a r') p = unit"
      using G.power_element_order G.power_mult \<open>a\<in>G\<close> less.prems(3) by (metis dvdE mult.commute)
    have fin: "finite G" using less by simp
    have aG: "G.power a r' \<in> G" using a by simp
    have dvd_p: "G.element_order (G.power a r') dvd p"
      using G.element_order_dvd[OF fin aG] pow_eq by blast
    have False if "G.element_order (G.power a r') = 1"
    proof -
     have "G.element_order a dvd r'"
       using G.element_order_dvd G.power_1 G.power_element_order \<open>a\<in>G\<close> aG fin that by metis
      then show False
        using G.element_order_pos[OF fin \<open>a\<in>G\<close>] r' prime by simp
    qed
  ...
  next
    case not_dvd: False
    have fin: "finite G" using less by simp
    define C where "C \<equiv> G.cyclic_subgroup a"
    ...
    have "card C > 1"
      using G.element_order_pos G.power_1 G.power_element_order a(1,2) card_C fin
      by (metis One_nat_def antisym_conv3 not_less_eq)
    then have "card FactG < card G"
      using "*" \<open>1 < card FactG\<close>
      by (metis One_nat_def bot_nat_0.not_eq_extremum n_less_m_mult_n not_less_iff_gr_or_eq)
    then obtain g where g: "g \<in> FactG" "Monoid.element_order comp (some_elem (CG.Class unit)) g = p"
      using less(1) \<open>p dvd card FactG\<close> abel_FactG \<open>finite FactG\<close> \<open>1 < card FactG\<close> by blast
...
definition conjugation :: "'a \<Rightarrow> 'a \<Rightarrow> 'a"
  where "conjugation g x \<equiv> (if g \<in> G \<and> x \<in> G then g \<cdot> x \<cdot> inverse g else x)"
...
proof -
  ...
  show ?thesis
  proof qed (use center_subset center_def in blast)
qed
...
proof -
  \<comment> \<open>Partition @{term H} into fibers.\<close>
  define fiber where "fiber s \<equiv> {g \<in> G. \<eta> g = s}" for s
...
proof (induction "card G" arbitrary: G "(\<cdot>)" "\<one>" m rule: less_induct)
  ...
  next
    ...
    proof (cases "p dvd card G.center")
      case True
      \<comment> \<open>By Cauchy's theorem for abelian groups, the center contains an element of order @{term p}.\<close>
      ...
      \<comment> \<open>Build the cyclic subgroup @{text "N = \<langle>z\<rangle>"} of order @{term p}.\<close>
      ...
      proof -
        ...
        define y where "y \<equiv> G.power z ((p - 1) * i)"
      ...
      proof (rule NG.Left_equals_Right_coset_implies_normality)
        ...
        proof -
          ...
          next
            ...
            then obtain n i where "n \<in> N" "x = composition n g" "n = G.power z i"
              using N_alt by blast
            then show "x \<in> NG.Left_Coset g N"
              using NG.Left_Coset_memI power_comm by metis
          qed
        qed
      qed
      ...
      proof -
        ...
        obtain K' where K'_sub: "Subgroup K' FactG fcomp (some_elem (NG.Class unit))"
                  and K'_card: "card K' = p ^ (m - 1)"
        ...
        define H where "H \<equiv> {g \<in> G. NG.Class g \<in> K}"
        ...
        proof
          show "NG_hom.Ker \<subseteq> N"
            using NG.Normal_def NG_hom.Ker_image subsetI by blast
          show "N \<subseteq> NG_hom.Ker"
            using G.left_unit G.unit_closed NG.Class_eq NG.CongruenceI NG.sub NG_hom.Ker_memI
            by (metis subsetI)
        qed
    ...
    next
      ...
      have False if "\<forall>y \<in> G. y \<notin> G.center \<longrightarrow> p dvd card (conj.orbit y)"
      proof -
        have "p dvd card (G - G.center)"
          using conj.dvd_card_nonfixed_points[OF fin]
          by (simp add: G.center_eq_fixed_points that)
        with False show False
          by (metis G.center_subset card_Diff_subset card_mono dvd_diffD1 fin finite_subset p_dvd_G)
      qed
      then obtain y where yG: "y \<in> G" and y_not_center: "y \<notin> G.center"
      ...
      have cent_grp: "Group (G.centralizer y) composition unit"
        using G.centralizer_subgroup yG Subgroup_def by meson
      ...
      have "card (G.centralizer y) < card G"
        using G.centralizer_proper[OF yG y_not_center fin] .
      then obtain H where H_sub: "Subgroup H (G.centralizer y) composition unit"
                      and H_card: "card H = p ^ m"
        using cent_fin cent_grp less.hyps pa_dvd_cent by blast
      then show ?thesis
        using subgroup_transitive[OF H_sub] G.centralizer_subgroup[OF yG] by auto
    qed
  qed
qed
...
text \<open>Cauchy's theorem for arbitrary finite groups is now an immediate corollary of the
  ...
  group of prime order is cyclic, so any of its non-unit elements has order exactly @{term p}.)\<close>
...
proof -
  ...
  have "card H \<noteq> 1"
    using cardH prime by force
  then obtain g where gH: "g \<in> H" and gne: "g \<noteq> \<one>"
    using is_singleton_altdef is_singleton_iff_ex1[of H] by blast
  ...
  have "H.sub.element_order g \<noteq> 1"
    using fin gH gne power_element_order by force
  with gH H.subset show ?thesis
    using dvdp prime by (auto simp: prime_nat_iff)
qed
...
proof -
  ...
  next
    ...
    then obtain hx hy where "hx \<in> H" "x = conjugation g hx"
      and "hy \<in> H" "y = conjugation g hy" and "x \<cdot> y = conjugation g (hx \<cdot> hy)"
      using conjugate_subgroup_def conjugation_hom by auto
    then show "x \<cdot> y \<in> conjugate_subgroup g H"
      using conjugate_subgroup_def by blast
  next
...
proof -
  ...
  show "H \<subseteq> subgroup_product H K" "H \<subseteq> subgroup_product K H"
...
proof -
  ...
  next
    fix g assume "g \<in> subgroup_product H K"
    then obtain h k where hH: "h \<in> H" and kK: "k \<in> K" and geq: "g = h \<cdot> k"
      unfolding subgroup_product_mem_iff by auto
    show "invertible g"
      by (simp add: geq hH kK)
    have "inverse g = inverse k \<cdot> inverse h"
      by (simp add: geq hH inverse_composition_commute kK)
    moreover obtain ikK: "inverse k \<in> K" and ihH: "inverse h \<in> H"
      using hH kK by blast
    ultimately show "inverse g \<in> subgroup_product H K"
      using local.swap subgroup_product_mem_iff by metis
  qed
qed
...
definition normalizer :: "'a set \<Rightarrow> 'a set"
  where "normalizer H \<equiv> {g \<in> G. conjugate_subgroup g H = H}"
...
proof (rule subgroupI)
  ...
  finally show "inverse g \<in> normalizer H"
...
proof -
  ...
  show ?thesis
  proof (intro normal_subgroup.intro subgroup_of_group.intro sub_H_N N.sub.Group_axioms)
...
proof -
  ...
  then interpret product: Subgroup "subgroup_product H K" G "(\<cdot>)" \<one> .
  ...
  proof
    ...
    obtain "h \<in> normalizer K" and "k \<in> normalizer K"
      using Ksub conjugate_subgroup_self fin hH kK norm normalizer_mem_iff by auto
    then show "x \<in> normalizer K"
      using xeq N.sub_composition_closed by simp
  qed
  have product_N_sub: "Subgroup (subgroup_product H K) (normalizer K) (\<cdot>) \<one>"
    by (meson N.Subgroup_axioms product_sub product_subset_N subgroup_restrict)
  interpret KN: normal_subgroup K "normalizer K" "(\<cdot>)" \<one>
    by (simp add: Ksub fin normal_in_normalizer)
  ...
  have K_product_sub: "Subgroup K (subgroup_product H K) (\<cdot>) \<one>"
    by (meson K_product Ksub product_sub subgroup_restrict)
  have K_normal_product: "normal_subgroup K (subgroup_product H K) (\<cdot>) \<one>"
  proof (intro normal_subgroup.intro subgroup_of_group.intro K_product_sub product.sub.Group_axioms)
    ...
    proof
      ...
      have inverse_eq: "product.sub.inverse g = N.sub.inverse g"
        using gP product_N_sub by (metis Subgroup.subgroup_inverse_equality)
      show "product.sub.inverse g \<cdot> k \<cdot> g \<in> K"
        using KN.normal gP inverse_eq kK product_subset_N by auto
    qed
  qed
  then interpret KP: normal_subgroup K "subgroup_product H K" "(\<cdot>)" \<one> .
  have H_product_sub: "Subgroup H (subgroup_product H K) (\<cdot>) \<one>"
    by (meson H.subset Hsub Ksub product_sub subgroup_product_superset_left subgroup_restrict)
  ...
  have HIntK_H_sub: "Subgroup (H \<inter> K) H (\<cdot>) \<one>"
    using HIntK_product_sub H_product_sub subgroup_restrict by fastforce
  interpret HIntK: normal_subgroup "H \<inter> K" H "(\<cdot>)" \<one>
  proof (intro normal_subgroup.intro subgroup_of_group.intro HIntK_H_sub H.sub.Group_axioms)
    show "normal_subgroup_axioms (H \<inter> K) H (\<cdot>) \<one>"
      by (simp add: H_product_sub K_normal_product normal_subgroup.axioms(2)
          product.sub.normal_subgroup_intersection)
  qed
  ...
  qed
  have card_product: "card (subgroup_product H K) = card K * card iso.Fract_HK_K"
  ...
  have "card (subgroup_product H K) * card (H \<inter> K)
      = card K * (card (H \<inter> K) * card iso.Fract_H_HIntK)"
    by (simp add: card_product iso_card)
...
proof -
  ...
  have P_self_orbit: "\<forall>g \<in> P. conjugate_subgroup g P = P"
    using Sylow_subgroups_subgroup assms conjugate_subgroup_self by blast
  ...
  define k where "k \<equiv> multiplicity p (card G)"
  ...
  then have "QG.index = card G div (p ^ k)"
    using QG.lagrange assms(2) cardQ' by auto
  then have not_p_dvd_index: "\<not> p dvd QG.index"
    using multiplicity_decompose[of concl: p]
    using assms(1) cardG_pos k_def not_prime_unit by auto
  ...
  proof
    ...
    show "G \<subseteq> \<Union>L"
      using L_def Qsub.sub_unit_closed act.Left_Coset_memI right_unit by (metis UN_iff subsetI)
  qed
      ...
      proof (intro antisym subsetI)
        ...
        proof -
          ...
          also have "\<dots> = g1 \<cdot> r"
            by (metis Qsub.sub associative eq12 g1G invertible invertible_right_inverse iq1Q q1G right_unit)
  ...
  also have "\<dots> = (\<Sum>C\<in>L. card Q)"
    by (simp add: card_coset)
  finally have "card G = card Q * card L" by simp
  with cardG_pos have card_L: "card L = QG.index"
    using QG.lagrange by fastforce
  ...
  proof (rule ccontr)
    ...
    proof
      ...
      obtain g where gG: "g \<in> G" and Ceq: "C = g \<cdot>| Q" using \<open>C \<in> L\<close> unfolding L_def by auto
    ...
    define Lorbs where "Lorbs \<equiv> P_act.orbit ` L"
...
proof -
  ...
  define m where "m \<equiv> multiplicity p (card G)"
  have cardP: "card P = p ^ m" and cardQ: "card Q = p ^ m"
    using Sylow_subgroups_card P Q by (simp_all add: m_def)
  have product_sub: "Subgroup (subgroup_product P Q) G (\<cdot>) \<one>"
    by (rule normalizes_subgroup_product_is_subgroup[OF Psub.Subgroup_axioms Qsub.Subgroup_axioms norm])
  ...
  have "card (subgroup_product P Q) * card (P \<inter> Q) = card P * card Q"
    by (rule normalizes_subgroup_product_card[OF Psub.Subgroup_axioms Qsub.Subgroup_axioms norm fin])
  then have "card (subgroup_product P Q) * card (P \<inter> Q) = p ^ (m + m)"
    by (simp add: cardP cardQ monoid_mult_class.power_add)
  then have product_dvd_power: "card (subgroup_product P Q) dvd p ^ (m + m)"
    by (metis dvd_def)
  obtain i where i_le: "i \<le> m + m" and card_product: "card (subgroup_product P Q) = p ^ i"
    using divides_primepow_nat[OF prime] product_dvd_power by blast
  have p_i_dvd_G: "p ^ i dvd card G"
    using card_product by (simp add: productG.lagrange[OF fin])
  have "G \<noteq> {}"
    using Psub.subset Psub.sub_unit_closed by blast
  then have "card G > 0"
    using fin by (simp add: card_gt_0_iff)
  then have "multiplicity p (p ^ i) \<le> multiplicity p (card G)"
    using dvd_imp_multiplicity_le p_i_dvd_G by blast
  then have i_le_m: "i \<le> m" using prime by (simp add: m_def)
  ...
  then have "m \<le> i"
    using prime prime_gt_1_nat by force
  with i_le_m have exp_eq: "i = m" by auto 
  have P_eq_product: "P = subgroup_product P Q"
    by (simp add: P_product cardP card_product card_subset_eq exp_eq fin_product)
  ...
  show ?thesis
    using P_eq_product Q_product cardP cardQ finP by (metis card_subset_eq)
qed
...
proof -
  ...
  proof (rule equalityI)
    ...
    proof
      ...
      obtain g where gG: "g \<in> G" and gPQ: "conjugate_subgroup g P = Q"
        using Sylow_II[OF prime fin P Q] by blast
...
theorem 
  assumes prime: "prime p" and fin: "finite G"
  shows Sylow_subgroups_card_dvd: "card (Sylow_subgroups p) dvd card G"
    and Sylow_III: "p dvd card (Sylow_subgroups p) - 1"
proof -
  have fin_S: "finite (Sylow_subgroups p)"
    using Sylow_subgroups_orbit fin prime by (metis equals0I finite.emptyI finite_imageI)
  obtain P where P: "P \<in> Sylow_subgroups p"
    using Sylow_subgroups_nonempty fin prime by blast
  interpret Psub: Subgroup P G "(\<cdot>)" \<one>
    using Sylow_subgroups_subgroup[OF P] .
  interpret act: Group_Action G "(\<cdot>)" \<one> "\<phi>_Sylow p" "Sylow_subgroups p"
    using \<phi>_Sylow_action[OF prime fin] .
  ...
  show "card (Sylow_subgroups p) dvd card G"
    using dvd_def orbit_stab by blast
  have P_act_map:
  ...
  proof qed (use act.hom_unit act.hom_compose P_act_map in auto)
  have fixed_P: "P \<in> P_act.fixed_points"
    by (simp add: Group.conjugate_subgroup_self Group_axioms P P_act.fixed_point_char
        Psub.Subgroup_axioms \<phi>_Sylow_def fin)
  have "Q = P" if Qfixed: "Q \<in> P_act.fixed_points" for Q
  proof -
    ...
    have normQ: "\<forall>g \<in> P. conjugate_subgroup g Q = Q"
      using P_act.fixed_point_char Qfixed Qmem \<phi>_Sylow_def by auto
    show ?thesis
      by (rule normalizes_Sylow_subgroups_eq[OF prime fin P Qmem normQ])
  qed
  then have "P_act.fixed_points = {P}"
    using fixed_P by auto  
  moreover have "p dvd card (Sylow_subgroups p - P_act.fixed_points)"
  proof (rule P_act.dvd_card_nonfixed_points[OF fin_S])
    ...
    have orbit_ne: "P_act.orbit Q \<noteq> {Q}"
      using P_act.fixed_point_iff_orbit_singleton Q Qnot by presburger
    have orbit_card_ne: "card (P_act.orbit Q) \<noteq> 1"
      using P_act.orbit_self[OF Q] orbit_ne by (metis card_1_singletonE emptyE insert_iff)
    have orbit_dvd: "card (P_act.orbit Q) dvd p ^ multiplicity p (card G)"
      using P_act.orbit_stabilizer_card Q Sylow_subgroups_card[OF P] by force
    show "p dvd card (P_act.orbit Q)"
      using orbit_card_ne orbit_dvd prime prime_power_dvd_not_prime_dvd by blast
  qed
  ultimately show "p dvd card (Sylow_subgroups p) - 1"
    using P by auto
qed
