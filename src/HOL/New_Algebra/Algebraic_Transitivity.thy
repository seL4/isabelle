...
next
  case True
  define n where "n \<equiv> card B"
  define P where "P \<equiv> (\<lambda>i. y ^ i) ` {..n}"
  ...
  define d where "d \<equiv> (\<lambda>i. c (y ^ i))"
  ...
  define p where "p \<equiv> (\<Sum>i\<le>n. monom (d i) i :: 'a poly)"
...
proof -
  ...
  have BL: "B \<subseteq> L"
    using B vs.basis_def by blast
  have mod_span: "vs.mod.spanning B"
    using vs.basis_spanning[OF B] by (simp add: vs.spanning_iff_mod_spanning[OF finB BL])
  have spanL: "L \<subseteq> vs.span B"
    using mod_span vs.mod.spanning_def by force
...
proof -
  define E where "E \<equiv> eval_img K a"
  define L where "L \<equiv> eval_img E b"
  ...
  define da where "da \<equiv> ext_degree K a"
  define db where "db \<equiv> ext_degree E b"
  define B where "B \<equiv> (\<lambda>(j,i). a ^ i * b ^ j) ` ({..<db} \<times> {..<da})"
  ...
  proof
    ...
    obtain c where cK: "\<And>i j. i < da \<Longrightarrow> j < db \<Longrightarrow> c i j \<in> K"
      ...
      using tower_power_basis_span[OF K alg_a alg_b]
      using E_def L_def da_def db_def by blast
    ...
    then show "y \<in> KL.vs.span B" 
      unfolding  yeq' B_def     
      by (intro KL.sum_scale_in_span) (auto simp: prodL cK)
  qed
...
theorem simple_ext_algebraic:
  ...
  shows "algebraic_over K y"
proof -
  define L where "L \<equiv> eval_img K a"
  define d where "d \<equiv> ext_degree K a"
  ...
  proof
    fix z assume "z \<in> L"
    then obtain c where cK: "\<And>i. i < d \<Longrightarrow> c i \<in> K" and zc: "z = (\<Sum>i<d. c i * a ^ i)"
      using Subfield.power_basis_span[OF K alg_a] by (auto simp: L_def d_def)
...
proof
  show "0 \<in> algebraic_elements F" "1 \<in> algebraic_elements F"
    using Subfield.algebraic_over_self Subfield_def algebraic_elements_def sfF by blast+
next
  fix a b
...
lemma generated_algebraic:
  ...
  assumes sfF: "Subfield F"
    ...
    and x: "x \<in> generate_field (F \<union> A)"
  shows "algebraic_over F x"
proof -
  have FA: "F \<union> A \<subseteq> algebraic_elements F"
    by (simp add: Subfield.algebraic_over_self algA algebraic_elements_def sfF subsetI)
  ...
  show ?thesis
    using x algebraic_elements_def gen by blast
qed
