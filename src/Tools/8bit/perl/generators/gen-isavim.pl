...
if ($config_file eq "") {
    ...
    print "usage gen-isavim [-d -dd] configfile\n", 
...
#$vim_dir = &look_for_value('^\s*VIM_DIR\s*"(.*)"',"VIM_DIR");

#if ($vim_dir eq "") {
#    die "\ncan't find VIM_DIR  in configuration file\n";}


#if (!(-r $pack."/".$vim_dir && -w $pack."/".$vim_dir && -x $pack."/".$vim_dir)){
#    die "\nneed read, write and execute permission for directory VIM_DIR\n";}
...
########################
# change to directory VIM_DIR and open the file `isavim' 

chdir $pack."/".$vim_dir || die "can't cd to $vim_dir: $!\n";
...
printf(OUTFILE "\" -e \$ISAVIMDIR/initvim \$*\n");
...
# END of script
# 
print "\nconfiguration of isavim  properly terminated\n\n";
exit(0);
...
# translate an entry for `isavim' script
sub translate_entry{
